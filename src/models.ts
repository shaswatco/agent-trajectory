...
/** Per-million-token prices in USD, plus optional capacity. */
export interface ModelPrice {
  /** Context capacity in tokens, for agents that do not log their own. */
...
/** A loaded pricing table, keyed by normalized model id. */
...
function toPrice(value: unknown): ModelPrice | undefined {
 ...
 */
export function loadPricing(path: string = pricingPath()): PricingTable {
  let text: string
  try {
  ...
  for (const [id, value] of Object.entries(parsed as Record<string, unknown>)) {
    ...
    if (price !== undefined) table[normalizeModelId(id)] = price
  }
  return table
}

/** Token counts a cost is computed from. */
...
export function costOf(
  ...
  if (id === undefined || !isRealModel(id)) return undefined
  // A free tier is the one price knowable without configuration.
  if (isFreeTier(id)) return 0
  const price = pricing[normalizeModelId(id)]
  if (price === undefined) return undefined
  const per = (tokens: number | undefined, rate: number | undefined): number =>
    tokens === undefined || rate === undefined ? 0 : (tokens / 1_000_000) * rate
 ...
 */
export function contextWindowFor(id: string | undefined, pricing: PricingTable): number | undefined {
  if (id === undefined || !isRealModel(id)) return undefined
