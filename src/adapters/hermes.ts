...
function foldDocument(document: Record<string, unknown>, fallbackTime: number): Trajectory {
  ...
  for (const entry of messages) {
    ...
    switch (message['role']) {
      case 'user':
        turns += 1
        if (text.length > 0) push({ kind: 'user', label: 'user', text, time })
        break
      case 'assistant': {
        steps += 1
        ...
        for (const call of Array.isArray(calls) ? calls : []) {
          ...
          const named = recordAt(call as Record<string, unknown>, 'function')
          push({
            kind: 'tool',
            label: typeof named?.['name'] === 'string' ? named['name'] as string : 'tool',
...
export function hermesAdapter(root: string = hermesRoot()): Adapter {
  return {
    id: 'hermes',
    root,
    discover: () => {
