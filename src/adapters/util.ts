...
export function modifiedAt(path: string): number | undefined {
  try {
...
const trajectoryCache = new Map<string, CacheEntry>()
...
function cachedIn<T>(cache: Map<string, CacheEntry>, path: string, compute: () => T): T {
 ...
 */
export function cachedTrajectory<T>(path: string, compute: () => T): T {
 ...
 */
export function readTextPrefix(path: string, bytes = 131_072): string | undefined {
...
export function jsonlFilesIn(dir: string): string[] {
...
export function readJsonlPrefix(path: string): Record<string, unknown>[] {
 ...
 */
export function decodeZstdFrames(buffer: Buffer, maxFrames = Number.POSITIVE_INFINITY): string {
...
export function flatten(value: unknown): string {
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    if (typeof record['text'] === 'string') return flatten(record['text'])
    if (record['content'] !== undefined) return flatten(record['content'])
  }
  return ''
}

/** A numeric field of a record. */
export function numberAt(source: Record<string, unknown> | undefined, key: string): number | undefined {
...
export function previewArguments(raw: unknown): string {
